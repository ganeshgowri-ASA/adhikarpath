...
import { Loader2, CheckCircle, XCircle, X, InboxIcon, Download } from "lucide-react";

interface RequestRole {
  role: { id: string; roleName: string; roleOwner?: { id: string; name: string } };
}
...
export function RoleOwnerDashboardClient() {
  useSession();
  ...
  const fetchRequests = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/requests?type=pending-ro");
  ...
  const filteredRequests = requests.filter((r) => {
    ...
    if (activeTab === "PENDING") return r.status === "PENDING" || r.status === "L1_APPROVED";
    if (activeTab === "APPROVED") return r.status === "APPROVED" || r.status === "RO_APPROVED";
    if (activeTab === "REJECTED") return r.status === "REJECTED" || r.status === "RO_REJECTED";
    return true;
  });

  const exportCsv = () => {
    const headers = ["Request ID", "Requestor", "Target User", "System", "Type", "Roles", "Submitted", "Status"];
    const rows = filteredRequests.map((r) => [
      r.id.slice(0, 8),
      `${r.requestor.name} (${r.requestor.empId})`,
      `${r.targetUser.name} (${r.targetUser.empId})`,
      r.system.systemName,
      r.requestType,
      r.requestRoles.map((rr) => rr.role.roleName).join("; "),
    ...
    a.href = url;
    a.download = `role-owner-requests-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <button
          onClick={exportCsv}
          className="ml-4 flex items-center gap-2 px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors whitespace-nowrap"
        >
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {filteredRequests.map((req) => (
                <tr key={req.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    {(req.status === "PENDING" || req.status === "L1_APPROVED") ? (
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    RO_APPROVED: "bg-green-50 text-green-700",
    REJECTED: "bg-red-50 text-red-700",
    RO_REJECTED: "bg-red-50 text-red-700",
    PENDING: "bg-yellow-50 text-yellow-700",
    L1_APPROVED: "bg-blue-50 text-blue-700",
  };
  return (
