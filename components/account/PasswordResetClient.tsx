// /home/user/adhikarpath/components/account/PasswordResetClient.tsx
"use client";

import { useState, useEffect } from "react";
import { useSession } from "next-auth/react";
import { KeyRound, Check, Loader2, AlertCircle, X } from "lucide-react";
    ...
    try {
      const res = await fetch("/api/requests", {
        ...
        body: JSON.stringify({
          ...
          justification: "Password reset request",
        }),
      });
      if (!res.ok) throw new Error("Submission failed");
      setConfirmSystem(null);
      showToast(`Password reset request submitted for ${confirmSystem.system.systemName}.`);
    } catch {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5 flex items-center gap-4">
        <div className="w-11 h-11 rounded-lg bg-blue-50 flex items-center justify-center shrink-0">
          <KeyRound className="w-6 h-6 text-[#1565C0]" />
        </div>
        <div>
          <h2 className="text-base font-semibold text-gray-900">Reset System Password</h2>
          <p className="text-sm text-gray-500 mt-0.5">
            Select a system below to submit a password reset request. Your manager will be notified for approval.
          </p>
        </div>
      </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center">
                <KeyRound className="w-5 h-5 text-[#1565C0]" />
              </div>
              <div>
                <h3 className="text-base font-semibold text-gray-900">Confirm Password Reset</h3>
                <p className="text-sm text-gray-500">This will submit a reset request for approval.</p>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button
                ...
                disabled={submitting}
                className="flex-1 px-4 py-2.5 text-sm font-medium text-white bg-[#1565C0] rounded-lg hover:bg-[#003c8f] transition-colors flex items-center justify-center gap-2 disabled:opacity-60"
              >
                {submitting ? (
                  <>
      ...
      {/* Toast */}
      {toast.visible && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 bg-gray-900 text-white text-sm px-4 py-3 rounded-xl shadow-lg max-w-sm animate-fade-in">
